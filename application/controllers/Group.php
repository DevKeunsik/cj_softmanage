...
class Group extends CI_Controller
{
     ...
     * 전산실보관 신규등록
     */
    public function insert_jeonsan_keep()
    {
        ...
        if ($_POST) {
            ...
            if ($table == "jeonsan_moniter") {
                ...
                $this->group_model->insert_keep_moniter(
                    $m_company, $m_product_name, $m_model_code, $m_model_name, $m_identify, $m_produce_ym, $m_soft_num, $m_gian_num, $m_buy_day

                );

            } else if ($table == "jeonsan_pc") {
                $p_company = $this->input->post('p_company', TRUE);
                $p_model_name = $this->input->post('p_model_name', TRUE);
                $p_model_code = $this->input->post('p_model_code', TRUE);
                $p_produce_number = $this->input->post('p_produce_number', TRUE);
                ...
                $this->group_model->insert_keep_pc(
            ...
            } else if ($table == "jeonsan_keyboard") {
                $k_product_name = $this->input->post('k_product_name', TRUE);
                $k_identify = $this->input->post('k_identify', TRUE);
                $k_produce_ym = $this->input->post('k_produce_ym', TRUE);
                $k_gian_num = $this->input->post('k_gian_num', TRUE);
                ...
                $this->group_model->insert_keep_keyboard(
                    $k_product_name, $k_identify, $k_produce_ym, $k_gian_num, $k_buy_day
                );
            } else if ($table == "jeonsan_mouse") {
                $mu_product_name = $this->input->post('mu_product_name', TRUE);
                $mu_identify = $this->input->post('mu_identify', TRUE);
                $mu_produce_ym = $this->input->post('mu_produce_ym', TRUE);
                $mu_gian_num = $this->input->post('mu_gian_num', TRUE);
                ...
                $this->group_model->insert_keep_mouse(
                    $mu_product_name, $mu_identify, $mu_produce_ym, $mu_gian_num, $mu_buy_day
                );
            } else if ($table == "jeonsan_headset") {
                $h_product_name = $this->input->post('h_product_name', TRUE);
                $h_gian_num = $this->input->post('h_gian_num', TRUE);
                $h_buy_day = $this->input->post('h_buy_day', TRUE);

                $this->group_model->insert_keep_headset(
                    $h_product_name, $h_gian_num, $h_buy_day
                );
            } else if ($table == "jeonsan_cell") {
                $c_product_name = $this->input->post('c_product_name', TRUE);
                $c_gian_num = $this->input->post('c_gian_num', TRUE);
                $c_buy_day = $this->input->post('c_buy_day', TRUE);

                $this->group_model->insert_keep_cell(
            ...
            // 쓰기 폼 view 호출
            $this->load->view('list/jeonsan/j_item_register');
        }
    }
    ...
    public function jeonsan_moniter_delete()
    {
        $this->load->helper('alert');
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_moniter($j_idx);
    ...
    public function jeonsan_pc_delete()
    {
        ...
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_pc($j_idx);
    ...
    public function jeonsan_keyboard_delete()
    {
        ...
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_keyboard($j_idx);
    ...
    public function jeonsan_mouse_delete()
    {
        ...
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_mouse($j_idx);
    ...
    public function jeonsan_headset_delete()
    {
        ...
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_headset($j_idx);
    ...
    public function jeonsan_cell_delete()
    {
        ...
        // 게시물 번호에 해당하는 게시물 삭제
        $j_idx = $this->uri->segment(3);
        $return = $this->group_model->delete_keep_cell($j_idx);
