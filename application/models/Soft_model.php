...
class Soft_model extends CI_Model
{
    ...
    function get_etc_use_cnt($serial_num,$company){
        switch($company){
            ...
            case "아시아소프트":
            case "한양정보통신":
                $sql = "SELECT count(asia) AS 'CNT' 
                        from g_font where asia like 
                        '%${serial_num}%' ";
                break;
            case "어도비":
                $sql = "SELECT count(adobe_product) AS 'CNT' 
                        from g_adobe where adobe_product like 
                        '%${serial_num}%' ";
                break;
            case "인큐브테크":
                $sql = "SELECT count(product_number) AS 'CNT' 
                        from g_quark where product_number like 
                        '%${serial_num}%' ";
                break;
            case "이스트소프트":
                $sql = "SELECT count(alzip) AS 'CNT' 
                        from g_compress where alzip like 
                        '%${serial_num}%' ";
                break;
        }
        $res =  $this->db->query($sql)->row_array();
    ...
    function modify_soft_progress($params)
    {
        ...
        $sql = "
			...
			SET product_name =?, version =?, company =?, purpose =?, target =?,
			 compatibility =?, serial_num =?, package =?, license_numb =?,
    ...
    function modify_kaspersky($params)
    {
        ...
        $sql = "
			...
			SET product_name =?, version =?, company =?, purpose =?, compatibility =?, 
			 duration =?, serial_num =?, package =?, license_numb =?,
			  keep_place =?, remarks =?
			WHERE idx =? ;
        ";
